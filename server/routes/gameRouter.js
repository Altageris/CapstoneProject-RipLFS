...
router.get("/ready", async function (req, res, next) {
  ...
  let readyStatus = routerFunction.ready(req, res, rooms);
  // Check if readyStatus is not null and is a string
  if (typeof readyStatus === 'string') {
    ...
    console.log("Ready status is not a valid room ID");
    // Handle the case where readyStatus is null or not a string
  }
});
...
module.exports = { gameRouter, rooms , globalRoomId};
